#!/usr/bin/env python3
# dropball.py
# ROS2 Humble + DroneKit + YOLOv5 (torch.hub local repo) + HSV color label (RED/YELLOW/BLUE)
#
# IMPORTANT COLOR NOTE:
# - ROS2 cv_bridge with desired_encoding='bgr8' returns OpenCV BGR frames.
# - HSVColorClassifier.classify() in hsv_color.py uses cv2.COLOR_BGR2HSV, so pass ROI in BGR directly.
#
# This file is a rewritten, cleaned version of your current dropball.py. 【:contentReference[oaicite:0]{index=0}】
...
# Paths / Params
# -----------------------------
MODEL_PATH = "/home/legion/ai/yolov5/runs/train/circle_yolov5n_4162/weights/best.pt"
...
class ImageSubcriber(Node):
    def __init__(self, topic_name="/UAV/forward/image_raw", queue_size=2):
        ...
        # NOTE: This is BGR (because desired_encoding='bgr8')
    ...
    def cb_image(self, msg):
        ...
        # Cache latest frame as JPEG bytes
        try:
        ...
        # Republish debug only if ROS context is still valid
        if rclpy.ok():
            try:
                ...
                # during shutdown publisher context invalid
                pass
...
class YOLOv5TorchHubDetector:
    def __init__(
        ...
        self.model.classes = None  # or [0] if you only have one class

    @torch.no_grad()
    def detect_all(self, frame_bgr):
        """
        Return list detections.
        det = {"bbox":(x1,y1,x2,y2), "center":(cx,cy), "conf":float, "cls":int}
        ...
        for (x1, y1, x2, y2, conf, cls_id) in det:
            ...
            cy = int((y1 + y2) * 0.5)
            out.append(
                {"bbox": (x1, y1, x2, y2), "center": (cx, cy), "conf": float(conf), "cls": int(cls_id)}
            )
...
class DroneController(Node):
    def __init__(self, connection_str="tcp:127.0.0.1:5763", takeoff_height=4.5):
        ...
        # detector + HSV classifier + tracker
        self.detector = YOLOv5TorchHubDetector(MODEL_PATH, imgsz=320, conf=0.35, iou=0.45, max_det=10, device="cuda:0")
        self.color_classifier = HSVColorClassifier(color_threshold=0.05, debug=False)  # uses BGR2HSV internally
        ...
        self._window_name = "UAV Camera (circle + HSV)"
        self._last_dets = []
    ...
    def _det_loop(self):
        ...
        while rclpy.ok() and not self._stop_flag:
            ...
            try:
                dets = self.detector.detect_all(frame_bgr)
            except Exception as e:
                try:
    ...
    # --------------------------
    # Visualization (HSV label is added "phía trên" bbox text)
    # --------------------------
    def _viz_timer_cb(self):
        if self._stop_flag or not getattr(self, "_show_camera", False):
            return
        ...
        # --------------------------
        # HSV COLOR LABEL (phía trên) + bbox drawing for EACH circle
        # --------------------------
        for det in dets:
            ...
            # ROI must be BGR (because frame is BGR, and HSVColorClassifier uses BGR2HSV)
            roi = vis[y1:y2, x1:x2]
        ...
        # Error (use best detection = highest confidence)
        if len(dets) > 0:
            best = dets[0]
            cx, cy = best["center"]
            h, w = vis.shape[:2]
        ...
        try:
            ...
            # q / ESC: close window + stop recording
            if key in (27, ord("q")):
                if getattr(self, "_recording", False):
                    self._stop_recording()
    ...
    # --------------------------
    # Mission (kept as your original state-machine)
    # --------------------------
        ...
        try:
            ...
            while rclpy.ok() and not self._stop_flag:
                if self.mission_step == 1:
                    self.get_logger().info("[Step 1] Move forward (detect RED).")
                    ok = self.move_with_timer(direction="forward", duration=8.3, speed=1)
                    time.sleep(3)
                    if not ok:
                        break
                    self.mission_step = 2
                    ...
                    self.get_logger().info("[Step 2] Move forward (detect RED).")
                    ok = self.move_with_timer(direction="forward", duration=2.5, speed=1)
                    time.sleep(5)
                    if not ok:
                        break
                    self.mission_step = 3
                    ...
                    self.get_logger().info("[Step 3] Move left (detect YELLOW).")
                    ok = self.move_with_timer(direction="left", duration=2.5, speed=1)
                    time.sleep(5)
                    if not ok:
                        break
                    self.mission_step = 4
                    ...
                    self.get_logger().info("[Step 4] Move right (detect YELLOW).")
                    ok = self.move_with_timer(direction="right", duration=5, speed=1)
                    time.sleep(5)
                    if not ok:
                        break
                    self.mission_step = 5
                    ...
                    self.get_logger().info("[Step 5] Move left/forward (detect BLUE).")
                    ok = self.move_with_timer(direction="left", duration=2.5, speed=1)
                    time.sleep(5)
                    if not ok:
                        break
                    ...
                    self.get_logger().info("[Step 6] Move backward.")
                    ok = self.move_with_timer(direction="backward", duration=13.5, speed=1)
                    time.sleep(1)
                    if not ok:
                        break
                    self.mission_step = 7
