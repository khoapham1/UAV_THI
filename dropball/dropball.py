...
# Paths / Params
# -----------------------------
MODEL_CIRCLE_PATH = "/home/legion/couseroot/ardupilot_gazebo/dropball/models/best_v5s.pt"
MODEL_H_PATH = "/home/legion/couseroot/ardupilot_gazebo/dropball/models/best_H_v5s.pt"
...
class UltralyticsDetector:
    ...
    def detect_all(self, frame_bgr):
        ...
        for (x1, y1, x2, y2), cf, cid in zip(xyxy, confs, clss):
            ...
            cx = int((x1 + x2) * 0.5)
            cy = int((y1 + y2) * 0.5)
            out.append({"bbox": (x1, y1, x2, y2), "center": (cx, cy), "conf": float(cf), "cls": int(cid)})
...
class DroneController(Node):
    def __init__(self, connection_str="tcp:127.0.0.1:5763", takeoff_height=4):
        ...
        # 2 detectors (Ultralytics YOLO26). Can be disabled to keep camera+recording only.
        if DISABLE_YOLO:
            self.get_logger().warn("[YOLO] UltralyticsDetector is DISABLED (camera+record still works).")
            self.detector_circle = DummyDetector()
            self.detector_h = DummyDetector()
        else:
            # YOLO26 models you trained (imgsz=320)
            # device: "0" for GPU0, or "cpu"
            self.detector_circle = UltralyticsDetector(
                MODEL_CIRCLE_PATH, imgsz=320, conf=0.7, iou=0.35, max_det=10, device="0"
            )
            self.detector_h = UltralyticsDetector(
                MODEL_H_PATH, imgsz=320, conf=0.3, iou=0.45, max_det=5, device="0"
            )
